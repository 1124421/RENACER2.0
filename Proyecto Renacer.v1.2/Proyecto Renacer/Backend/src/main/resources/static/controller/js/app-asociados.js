...
async function inicializarAsociados() {
    ...
    window.guardarNuevoAsociado = async function(e) {
        ...
        const nuevoAsociado = {
            ...
            carreta: document.getElementById('newCarreta')?.value.trim() || '',
            ruta: document.getElementById('newRuta')?.value.trim() || '',
            idUnico: document.getElementById('newIdUnico')?.value.trim() || '',
            tipoAsociado: document.getElementById('newTipoAsociado')?.value.trim() || '',
            ingresos: 0
        };
        ...
        if (typeof window.API !== 'undefined') {
            try {
                ...
                const asociadoParaAPI = {
                    ...
                    tipo: nuevoAsociado.tipoAsociado === 'Aforado' ? 'Aforado' : 'No_aforado',
                    carreta: nuevoAsociado.carreta || null,
                ...
                asociadosActuales.push({
                    ...
                    ingresos: 0,
                    carreta: creado.carreta || nuevoAsociado.carreta || '',
                    ruta: nuevoAsociado.ruta || '',
        ...
        ['newNombre', 'newDocumento', 'newContacto', 'newCorreo', 'newCarreta', 'newRuta', 'newIdUnico', 'newTipoAsociado'].forEach(id => {
            const el = document.getElementById(id);
            if (el) el.value = '';
        });
